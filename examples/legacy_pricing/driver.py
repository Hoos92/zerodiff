"""Deterministic driver: exercises the legacy pricing module under recording.

Instruments boundaries with retrace.wrap() — the legacy source is never
edited. Run via:
...
import pricing  # noqa: E402  (wrapped above)
...
def scenarios():
    ...
    yield {"items": [{"sku": "NT", "qty": 2, "unit_price": 7.5}]}  # no tier key
...
def main():
    ...
    for tier in TIERS + ["Gold"]:
        ...
        try:
            ...
            failures += 1
    print("driver: made %d top-level calls (%d raised, as expected)"
          % (calls, failures))
