...
## Do I need an LLM to use ZeroDiff?
...
produce identical results on every run. You can use ZeroDiff exactly like
you use pytest:

- run `zerodiff record` / `zerodiff replay` by hand or in CI,
- gate merges with the GitHub Action or `zerodiff.testing.verify_traces()`,
- read `zerodiff-report.md` yourself and fix divergences yourself.

Where an LLM *optionally* enters is on the **fixing** side — when you want
something to write or repair the rewrite for you. ZeroDiff supports three
levels of that, all vendor-neutral:

1. **You use your own agent, manually.** Ask Claude Code / Cursor /
   Copilot to fix the code and paste or point it at `zerodiff-report.json`
   — every divergence carries a hint written for exactly this purpose.
2. **Your agent calls ZeroDiff natively.** Register the MCP server
   (`claude mcp add zerodiff -- zerodiff mcp`) and the agent can run
   `zerodiff_replay` itself and read the results, mid-task. The Claude Code
   hook variant blocks any edit that breaks recorded behavior.
3. **ZeroDiff drives the agent, unattended.**
...
ZeroDiff itself never calls a model. Your traces never leave your machine.
...
- `--llm provider:model`: ZeroDiff's **built-in minimal agent** calls the
  ...
  needed. Validate a key/model in seconds with `zerodiff llm-check`.
...
No, and ZeroDiff never claims it is. It proves equivalence **over the
...
behaviors" and list per-boundary counts; anything ZeroDiff couldn't fully
compare is flagged (`weak_comparison`, `skipped`) rather than silently
counted as a match.

## Is my data safe?

Traces are plain local files and may contain real runtime data. ZeroDiff
never uploads anything. `zerodiff init` gitignores the traces directory by
default; `redact_fields` in `zerodiff.toml` strips named fields **at record
...
`zerodiff.wrap("mod", "Class.method")`) whose inputs are reasonably
...
recorded reality — see [VALIDATION.md](VALIDATION.md). ZeroDiff complements
