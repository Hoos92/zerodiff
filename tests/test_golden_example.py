...
def _run(cwd, *args):
    return subprocess.run(
        [sys.executable, "-m", "retrace.cli"] + list(args),
...
def demo_dir(tmp_path_factory):
    ...
    shutil.copytree(EXAMPLE, target, ignore=shutil.ignore_patterns(
        "traces", "retrace-report.*", "__pycache__"))
...
def test_equivalent_rewrite_matches_everything(demo_dir):
    ...
    with open(os.path.join(demo_dir, "retrace-report.json"),
              encoding="utf-8") as f:
        report = json.load(f)
    s = report["summary"]
...
def test_report_md_renders_honestly(demo_dir):
    ...
    md_path = os.path.join(demo_dir, "retrace-report.md")
