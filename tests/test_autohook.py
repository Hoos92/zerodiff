...
from zerodiff.autohook import instrument_module, matches
...
class TestInstrumentModule:
    ...
    def test_wraps_public_functions_only(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            assert count == 1
            assert getattr(module.public, "__zerodiff_wrapped__", None) \
                is not None
            assert getattr(module._private, "__zerodiff_wrapped__", None) \
                is None
            # imported names must not be wrapped
            assert getattr(module.os.path.join, "__zerodiff_wrapped__",
                           None) is None
        finally:
            cleanup()
...
def test_record_include_needs_no_source_edits(tmp_path):
    ...
    --include with zero zerodiff imports anywhere in user code."""
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "zerodiff.cli", "record", "-o", "traces",
