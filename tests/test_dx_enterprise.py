...
import retrace
from retrace import enterprise, report as report_mod
from retrace.scaffold import cmd_init
from retrace.testing import BehaviorMismatch, verify_traces
...
class TestInit:
    def test_creates_config_and_gitignore(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        assert cmd_init() == 0
        config = (tmp_path / "retrace.toml").read_text(encoding="utf-8")
    ...
    def test_never_overwrites_existing_config(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / "retrace.toml").write_text('[map]\n"a" = "b"\n',
                                               encoding="utf-8")
        cmd_init()
        assert '"a" = "b"' in (tmp_path / "retrace.toml").read_text(
            encoding="utf-8")


def test_demo_runs_end_to_end():
    proc = subprocess.run([sys.executable, "-m", "retrace.cli", "demo"],
...
class TestAttestation:
    ...
    def test_sign_and_verify(self, project):
        ...
        attestation = enterprise.build_attestation(
            "traces", "retrace-report.json", key)
