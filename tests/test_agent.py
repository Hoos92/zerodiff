...
import zerodiff
from zerodiff import cli
from zerodiff.agent import AgentError, BuiltinAgent, parse_llm_spec
...
def ws(tmp_path, monkeypatch):
    ...
    for env in ("OPENAI_API_KEY", "ZERODIFF_LLM_API_KEY"):
...
def _block(name, content):
    return "<<<ZERODIFF-FILE: %s>>>\n%s<<<ZERODIFF-END>>>" % (name, content)
