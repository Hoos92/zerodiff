...
import zerodiff
from zerodiff import cli, differ, enterprise, recorder, report as report_mod
...
class TestMutationCapture:
    def _record(self, ws_dir):
        ...
        zerodiff.wrap("v8leg_m", "register")
        zerodiff.start_recording(str(ws_dir / "traces"))
        try:
    ...
    def test_opt_out_disables_capture(self, ws, monkeypatch):
        (ws / "zerodiff.toml").write_text("[record]\nmutations = false\n",
                                         encoding="utf-8")
        monkeypatch.setattr(recorder, "_config", None)
        self._record(ws)
        trace_file = next((ws / "traces").glob("*.jsonl"))
...
class TestInOrderReplay:
    def _record_interleaved(self, ws_dir):
        ...
        zerodiff.wrap("v8state_a", "push")
        zerodiff.wrap("v8state_a", "total")
        zerodiff.start_recording(str(ws_dir / "traces"))
        try:
...
def test_mcp_quality_tool(ws):
    ...
    proc = subprocess.run([sys.executable, "-m", "zerodiff.mcp_server"],
...
class TestAttestationCoherence:
    ...
    def test_env_var_key_and_quality_block(self, ws, monkeypatch):
        self._setup(ws)
        monkeypatch.setenv("ZERODIFF_ATTEST_KEY",
                           "env-provided-signing-key-123")
        attestation = enterprise.build_attestation(
            "traces", "zerodiff-report.json", None,
