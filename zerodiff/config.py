"""Loading ``zerodiff.toml``.

Uses the stdlib ``tomllib`` on Python 3.11+, and falls back to a minimal
built-in reader for the subset of TOML that zerodiff configs use (sections,
...
DEFAULT_CONFIG_NAME = "zerodiff.toml"
...
def _parse_value(text: str, where: str) -> Any:
    ...
    try:
        ...
        raise ValueError(
            "zerodiff.toml: cannot parse value %r (%s). The built-in reader "
...
def _parse_toml_subset(text: str, path: str) -> Dict[str, Any]:
    ...
    for lineno, raw_line in enumerate(text.splitlines(), 1):
        ...
        if line.startswith("[["):
            ...
            # falling through.
            raise ValueError(
                "zerodiff.toml: [[array-of-tables]] syntax is not supported "
        ...
        if m:
            ...
            for part in _split_dotted(m.group(1)):
                ...
                if not isinstance(current, dict):
                    raise ValueError("zerodiff.toml: section conflicts with "
                                     "existing key (%s)" % where)
            continue
        if "=" not in line:
            raise ValueError("zerodiff.toml: expected 'key = value' (%s)" % where)
