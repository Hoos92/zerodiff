# ZeroDiff + Claude Code

Two ways to wire ZeroDiff into Claude Code, from loosest to strictest.
...
Claude Code then has three tools: `zerodiff_replay` (replay traces against
the current code, get divergences with hints), `zerodiff_report` (read an
existing report), and `zerodiff_quality` (run the security/quality gate over
files it just wrote, before replaying). Ask Claude
to "verify the rewrite with zerodiff" and it can run and read the results
...
`zerodiff_hook.py` replays the recorded traces after each Edit/Write. On
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python integrations/claude-code/zerodiff_hook.py"
          }
        ]
      }
    ]
...
## 3. Unattended loop (ZeroDiff drives the agent)

```bash
zerodiff loop -t traces --agent "claude -p --permission-mode acceptEdits" --max-iters 5
```
